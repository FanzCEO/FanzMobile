...
export function registerRoutes(app: Express) {
  ...
  app.get('/api/privacy/export', csrfProtection, isAuthenticated, async (req, res) => {
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=fanzmobile-data-export-${userId}.json`);
      res.json(exportData);
    } catch (error) {
  ...
  app.get('/api/compliance/custodian-notice', async (req, res) => {
    try {
      const custodianInfo = {
        ...
        notice: `Pursuant to 18 U.S.C. Section 2257, the following individual has been designated as the custodian of records for FanzMobile platform:`,
        custodian: {
          name: 'FanzMobile Legal Compliance Officer',
          company: 'FanzMobile LLC',
          address: {
