...
import { useAuth, useUserProfile, useCreatorProfiles } from "@/hooks/useSupabase";
...
export default function MobileProfile({ onPageChange }: MobileProfileProps) {
  ...
  const { data: creatorProfiles } = useCreatorProfiles(user?.id);
  const { isDark, toggleTheme } = useTheme();
  
  const [profileStats, setProfileStats] = useState([
    { label: "Total Earnings", value: "$0", icon: "💰" },
    { label: "Total Followers", value: "0", icon: "👥" },
    { label: "Connected Profiles", value: "0", icon: "🔗" },
    { label: "Protected Content", value: "0", icon: "🛡️" }
  ]);

  // Get creator ID from profiles
  const creatorId = creatorProfiles?.[0]?.id || userProfile?.id;
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        const [earnings, followers, protectedCount] = await Promise.all([
          db.creators.getEarningsSummary(creatorId),
          db.followers.getCount(creatorId),
          db.content.getProtectedCount(creatorId)
        ]);

        const totalEarnings = earnings.data?.total_earnings || 0;
        const totalFollowers = followers.count || 0;
        const protectedContentCount = protectedCount.count || 0;
        const connectedProfiles = platformManager.getConnectedProfiles();

        setProfileStats([
          { label: "Total Earnings", value: `$${totalEarnings.toLocaleString()}`, icon: "💰" },
          { label: "Total Followers", value: totalFollowers >= 1000 ? `${(totalFollowers / 1000).toFixed(1)}K` : totalFollowers.toString(), icon: "👥" },
          { label: "Connected Profiles", value: connectedProfiles.length.toString(), icon: "🔗" },
          { label: "Protected Content", value: protectedContentCount.toString(), icon: "🛡️" }
        ]);
      } catch (error) {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {/* Theme Toggle Card - Prominent placement */}
      <Card className="border-2 border-primary/20 bg-gradient-to-r from-primary/5 to-secondary/5">
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 dark:from-blue-600 dark:to-purple-600 flex items-center justify-center text-white">
                {isDark ? "🌙" : "☀️"}
              </div>
              <div>
        ...
        <CardContent className="p-4">
          <div className="flex items-center space-x-4">
            <Avatar className="h-16 w-16">
              <AvatarImage src={userProfile?.avatar_url || "/avatars/01.png"} alt={userProfile?.handle || "user"} />
              <AvatarFallback>
                {(userProfile?.handle || user?.email || "U").substring(0, 2).toUpperCase()}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1">
              <h2 className="text-lg font-semibold">
                {userProfile?.handle ? `@${userProfile.handle}` : user?.email?.split("@")[0] || "User"}
              </h2>
